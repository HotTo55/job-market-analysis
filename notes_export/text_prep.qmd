...
```{python}
lovely_string = ["loved", "lover", "lovely", "loving", "loveliest", "lovable", "lovelier"]
...
lyrics_pd = pd.read_csv(
  '/Users/sberry5/Documents/teaching/UDA/code/lyrics_scrape_python/complete_lyrics_2025.csv'
)
...
hfl_pd['lyrics'] = (
  hfl_pd.lyrics.str.replace('(\\[.*?\\])', '', regex=True) 
...
copperhead_pd['lyrics'] = (
  copperhead_pd.lyrics.str.replace('(\\[.*?\\])', '', regex=True) 
  .str.replace('([a-z])([A-Z])', '\\1 \\2', regex=True) 
)
```
...
choctaw_pd['lyrics'] = (
  choctaw_pd.lyrics.str.replace('(\\[.*?\\])', '', regex=True) 
  .str.replace('([a-z])([A-Z])', '\\1 \\2', regex=True) 
)
```
...
df_countvect = pd.DataFrame(data = songs_tfidf.toarray(), index = ['HFL', 'Copperhead', 'Choctaw'],
